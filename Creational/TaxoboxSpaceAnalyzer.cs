...
{
    // can be a template title or a page title
    public String Title { get; set; }

    public Boolean IsSpecies { get; set; }

    public WikiPage Page { get; set; }

    public LiveTreeNode Parent { get; set; }

    public List<LiveTreeNode> Children { get; set; }
}


public class LiveTreeBuilder
{
    static Logger log = LogManager.GetCurrentClassLogger();

    private readonly IDbContextFactory<ApplicationDb> dbContextFactory;

    public LiveTreeBuilder(IDbContextFactory<ApplicationDb> dbContextFactory)
    {
        this.dbContextFactory = dbContextFactory;
    }

    LiveTreeNode Build(String lang)
    {
        var rootTitle = "Life";

        var db = dbContextFactory.CreateDbContext();

        var taxoTemplatesByTitle = db.Pages
            .Include(p => p.Parsed)
                .ThenInclude(p => p.TaxoTemplateValues)
            .Where(p => p.Type == PageType.TaxoTemplate && p.Lang == lang)
            .ToDictionary(p => p.Title)
            ;

        var speciesPages = db.Pages
            .Include(p => p.Parsed)
                .ThenInclude(p => p.TaxoTemplateValues)
            .Where(p => p.Type == PageType.Content && p.Lang == lang && p.Parsed.TemplateName == "speciesbox")
            .ToDictionary(p => p.Title)
            ;

        if (!taxoTemplatesByTitle.TryGetValue(rootTitle, out var rootTemplate))
        {
            throw new Exception($"Can't find root {rootTitle}");
        }
...
public class TaxoboxSpaceAnalyzer
{
    ...
    public void AnalyzeSpecies(String lang)
    {
        ...
        (WikiPage template, String error) FindTaxoTemplate(SpeciesValues speciesValues)
        {
        ...
        var groupedByError = (
            ...
            let p = FindTaxoTemplate(s.SpeciesValues)
